...
const clinicsData = [
    {
      ...
      categoriesArray: ["Dermatology","Cardiology"]
    },
    {
      name: "Rahul Clinic",
      shortAddress: "abu dabi",
      city: "dubai",
      contactNumber: 8767213959,
      ratings: 4.8,
      images: [cImage1, cImage2, cImage3],
      categoriesArray: ["Neurology","Dermatology", "Dentist","Cardiology"]
...
const SingleCategoryScreen = ({route}) => {
    ...
    clinicsData.map((it, id) => {it.categoriesArray.map((item,idx) => {if(item.includes(`${category}`)){
        obj.push(it)
    }})})
